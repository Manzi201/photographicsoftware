...
// ── Fixed salt — MUST match staffAuthController.js ────────────
...
exports.createStaff = async (req, res) => {
  try {
    ...
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (existing) {
      return res.status(400).json({ success: false, error: 'Username already taken in this school' });
    }

    const password_hash = hashPassword(password); // ← fixed-salt, no school_id arg
    ...
    const { data, error } = await supabase
      ...
      .select('id,full_name,email,phone,role,username,is_active,created_at')
      .single();
    if (error) throw error;
...
exports.updateStaff = async (req, res) => {
  try {
    ...
    if (password) update.password_hash = hashPassword(password); // ← fixed-salt
    ...
    const { data, error } = await supabase
      ...
      .eq('id', req.params.id)
      .eq('school_id', schoolId)
      .select('id,full_name,email,phone,role,username,is_active')
      .single();
    if (error) throw error;
    res.json({ success: true, data });
  } catch (err) {
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    const hash = hashPassword(new_password); // ← fixed-salt
    await supabase.from('staff')
      .update({ password_hash: hash })
      .eq('id', req.params.id)
      .eq('school_id', schoolId);
    // Invalidate all sessions for this staff
    await supabase.from('staff_sessions').delete().eq('staff_id', req.params.id);
