...
import { UserPlus, Edit2, Trash2, X, Check, Key, Shield } from 'lucide-react';
...
const ROLE_COLORS = { admin:'bg-red-100 text-red-700', dos:'bg-purple-100 text-purple-700', teacher:'bg-blue-100 text-blue-700', secretary:'bg-green-100 text-green-700', finance:'bg-amber-100 text-amber-700' };
...
export default function AdminStaff() {
  ...
  const byRole = Object.keys(ROLES).reduce((acc,r) => ({ ...acc, [r]: staff.filter(s=>s.role===r) }), {});
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {/* Staff list by role */}
      {Object.entries(ROLES).map(([role, roleLabel]) => {
      ...
      {modal && (
        <StaffModal staff={modal==='new'?null:modal} onSave={()=>{setModal(null);loadStaff();}} onClose={()=>setModal(null)}/>
      )}
      ...
      {resetId && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-80 shadow-2xl">
            ...
            <input type="password" className="input-field mb-4" value={newPwd} onChange={e=>setNewPwd(e.target.value)} placeholder="New password (min 6 chars)" autoFocus/>
